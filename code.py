...
class Blockchain():
    ...
    def new_transaction(self, sender_id, recipient_id, product_code, price):
        ...
        if product_code in product_codes:
            ...
            transactions[product_code].append(transaction)#-->>
            # self.current_transactions.append(transaction)
...
@app.route('/getprotransactions', methods=['POST'])
def getpro_transactions():
    values = request.get_json()
    required = ['product_code']
    if not all(k in values for k in required):
        return 'Missing values', 400

    protransaction_list = transactions[values['product_code']]
    pro_transaction_list = []
    for i in protransaction_list:
        pro_transaction_list.append(i)

    response = { "transactions": pro_transaction_list}
    return jsonify(response), 201
